...
const frameworkHash = frameworkFingerprint(root, ['scripts/suggest.js', 'scripts/lib/note-rules.js', 'scripts/lib/note-enhancements.js', 'data/note-enhancements.json', 'data/java-concepts.js', 'data/chapter-status.json']);
const chapterFingerprint = chapter => hash([frameworkHash, chapter.topics.map(t => [t.filePath, t.code])]);
...
if (reviewedArg !== -1 && process.argv[reviewedArg + 1]) {
  ...
  const content = concepts.find(c => c.name === chapter);
  if (!content) throw new Error(`Unknown chapter: ${chapter}`);
  state.reviewedChapters[chapter] = { reviewedAt: new Date().toISOString(), sourceFingerprint: chapterFingerprint(content) };
  writeState();
  console.log(`Marked this source/framework version as reviewed: ${chapter}`);
  process.exit(0);
}
...
for (const chapter of concepts) {
  ...
  if (state.reviewedChapters[chapter.name] && state.reviewedChapters[chapter.name].sourceFingerprint === chapterFingerprint(chapter)) continue;
  reviewedCount++;
  ...
  for (const topic of chapter.topics || []) {
    ...
    for (const entry of JDK_APIS) {
      ...
      suggestions.push({
        ...
        file: topic.filePath, entry, why: `heuristic: code mentions ${entry.note}; no ${simple} token was found in this chapter's notes`
      });
    }
    ...
    for (const entry of CONSTRUCTS) {
      ...
      suggestions.push({
        ...
        file: topic.filePath, entry, why: `heuristic: code matches ${entry.name}; its configured explanatory terms were not found in the notes`
      });
    }
  }
}
...
const output = suggestions.map(s => ({
  ...
  why: s.why,
  evidence: 'heuristic',
  sourceFingerprint: chapterFingerprint(concepts.find(c => c.name === s.chapter)),
  contentVersion: hash([s.key, s.why, draftNote(s), frameworkHash]),
  draft: draftNote(s)
}));

const editorial = require('./lib/note-enhancements');
const noteReview = editorial.analyze(concepts, require('../data/note-enhancements.json'));
output.push(...noteReview.proposals.filter(item => !state.rejected[item.key]));
fs.writeFileSync(path.join(dashboardDir, 'note-quality.json'), JSON.stringify(noteReview, null, 2) + '\n');
fs.writeFileSync(path.join(dashboardDir, 'note-quality.md'), editorial.markdown(noteReview));

fs.writeFileSync(
  path.join(dashboardDir, 'suggestions-data.js'),
  `// Auto-generated by scripts/suggest.js. Do NOT edit — run 'npm run revise'.\nconst SUGGESTIONS = ${JSON.stringify({ schemaVersion: 2, sourceFingerprint: hash(concepts.map(chapterFingerprint)), frameworkFingerprint: frameworkHash, items: output }, null, 2)};\n`,
  'utf8'
);

const md = ['# Suggested Enhancements', '',
...
for (const item of output) {
  if (item.kind === 'editorial') {
    md.push(`## ${item.chapter}: ${item.topic}`, '', `**${item.what}** (${item.category})`, '', item.why, '',
