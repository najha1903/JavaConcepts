...
const { runProcess, mapWithConcurrency, availableConcurrency, requireJava, createWorkDir, nativeValidation } = require('./lib/run-java.js');
const { hash, frameworkFingerprint } = require('./lib/content-identity.js');

const root = path.resolve(__dirname, '..');
const quiet = process.argv.includes('--quiet');
const toolchain = requireJava();
const validatorFingerprint = frameworkFingerprint(root, ['scripts/fill-practice-expectations.js', 'scripts/lib/lab-verifier.js', 'scripts/lib/run-java.js', 'scripts/lib/note-rules.js', 'revision-dashboard/app.js', 'data/practice-contracts.js', 'data/chapter-status.json']);
const validation = nativeValidation('scripts/fill-practice-expectations.js', validatorFingerprint);
...
function needsFilling(challenge) {
  if (challenge.evidence && ['independent-contract', 'authored-note'].includes(challenge.evidence.expectation)) return false;
...
function fingerprint() {
  ...
  feed(JSON.stringify(challenges.map(c => [c.id, c.methodName, c.sourceFile, c.template])));
  feed(validatorFingerprint);
  feed(JSON.stringify(toolchain));
...
function sourceFor(challenge) {
  if (challenge.sourceFile) {
    return concepts.flatMap(chapter => chapter.topics || []).find(topic => topic.filePath === challenge.sourceFile) || null;
  }
...
const work = createWorkDir('expect');
...
async function processChallenge(challenge) {
  ...
  for (const args of argSets) {
    ...
    if (!trimmed && !isVoid) return { kind: 'noResult' };
    cases.push({ args, expected: asJsValue(trimmed, returnType), provenance: {
      kind: 'solution-derived-smoke', sourceFile: topic.filePath, sourceHash: hash(topic.code),
      frameworkFingerprint: sourceFingerprint, execution: 'native-reference-and-browser-approximate', nativeValidation: validation
    } });
  }
...
for (let i = 0; i < outcomes.length; i++) {
  ...
  if (existing[challenge.id]) throw new Error(`${challenge.id}: previously executable smoke check now failed (${outcome.kind}); stale values were not published.`);
}
