...
function validateReplacement(source, before, after) {
  ...
  if (start < 0 || source.indexOf(before, start + 1) !== -1) {
  ...
  if (!blocks.some(b => start >= b.start && start < b.end) ||
      !blocks.some(b => start + before.length <= b.end && start + before.length > b.start)) {
  ...
  if (codeOnly(source) !== codeOnly(next)) throw new Error('An enhancement must not change executable Java.');
  return next;
}
    ...
    for (const topic of chapter.topics) {
      ...
      const propose = (entry, before, after, line) => {
        ...
        proposals.push({ key, kind: 'editorial', chapter: chapter.name, topic: topic.topicName,
          ...
          sourceFingerprint: hash(source), contentVersion: hash([before, after]),
