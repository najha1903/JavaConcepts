...
| `check-dashboard.js` | Opens the dashboard in a real DOM and drives it. Every view renders with no `NaN` and no literal "undefined"; the chapter being written is silent and refuses a quiz; a finished chapter quizzes normally and its header count matches; no quiz repeats a question; Quick Revision shows the cram sheet with snippets as code; Mastery routes into the Bank; each preset selects exactly what its filter selects; and none of the tool's old invented text is on screen. Skips cleanly if jsdom is not installed. |
...
**The checks read the files; one of them runs the page.** `check-dashboard.js` is the only check that opens the dashboard in a DOM and clicks things, and it catches a class of failure none of the others can see: a function renamed, an element id changed, a view that throws halfway through rendering, a count that prints `NaN`, a button wired to a handler that no longer exists. Every one of those leaves the generated data perfectly valid. It needs `jsdom`, which is a devDependency; if it is missing the check **skips** rather than fails, because a missing tool is not a defect in the notes.
